...
const T = {
  uk: { search: 'Пошук по меню...', all: 'Все', items: 'позицій', nothing: 'Нічого не знайдено', loading: 'Завантаження...', wifi: 'Вільний доступ', review: 'Залишити відгук' },
  en: { search: 'Search menu...', all: 'All', items: 'items', nothing: 'Nothing found', loading: 'Loading...', wifi: 'Free access', review: 'Leave a review' },
};

function BadgeList({ badges = [], lang = 'uk' }) {
  ...
  return (
    <div className={s.badges}>
      {badges.map(id => {
        ...
        if (!b) return null;
        return (
          <span key={id} className={s.badge} style={{ background: b.bg, color: b.color }}>
            {b.emoji} {lang === 'en' ? b.labelEn : b.labelUk}
          </span>
        );
      })}
    </div>
...
export default function Menu({ menuData, loading, onAdminClick }) {
  ...
  useEffect(() => {
    if (sections.length) {
      const init = {};
      sections.forEach((sec, i) => { if (i < 3) init[sec.id] = true; });
      setOpen(init);
    }
  ...
  const shown = sections
    ...
    .map(sec => ({
      ...
      items: sec.items.filter(i =>
        i.visible && (!search || name(i).toLowerCase().includes(search.toLowerCase()))
      ),
    }))
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.searchBox}>
        ...
        <input className={s.input} placeholder={t.search} value={search} onChange={e => setSearch(e.target.value)} />
      ...
      <div className={s.body}>
        ...
        {shown.map(sec => (
          <div key={sec.id} className={s.sec}>
            <button
              className={`${s.secBtn} ${open[sec.id] ? s.secOpen : ''}`}
              onClick={() => setOpen(p => ({ ...p, [sec.id]: !p[sec.id] }))}
            >
              <div className={s.secL}>
                <span className={s.secIco}>{sec.icon}</span>
                <div>
                  <div className={s.secName}>{title(sec)}</div>
                  <div className={s.secCnt}>{sec.items.filter(i => i.visible).length} {t.items}</div>
                </div>
              </div>
                ...
                {sec.items.map((item, idx) => (
                  ...
                  >
                    <div className={s.cardImg}>
                      {item.photo
                        ? <img src={item.photo} alt={name(item)} loading="lazy"
                            onError={e => { e.target.style.display = 'none'; e.target.parentNode.textContent = sec.icon; }} />
                        : <span>{sec.icon}</span>}
                    </div>
                    <div className={s.cardBody}>
                      ...
                      <BadgeList badges={item.badges} lang={lang} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {modal && (
        <div className={s.overlay} onClick={() => setModal(null)}>
          <div className={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <BadgeList badges={modal.item.badges} lang={lang} />
