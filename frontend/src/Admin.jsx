...
export default function Admin({ menuData, setMenuData, onClose, onPrint }) {
  ...
  const login = () => {
    ...
    if (hash(pw) === HASH) { sessionStorage.setItem('pu_adm', '1'); setAuthed(true); setErr(''); }
    else {
      const t = tries + 1; setTries(t); setPw('');
      if (t >= 5) { setLock(Date.now() + 30000); setTries(0); setErr('Заблоковано 30 сек'); }
      else setErr(`Невірний пароль · Залишилось: ${5 - t}`);
    }
  };
  const logout = () => { sessionStorage.removeItem('pu_adm'); setAuthed(false); onClose(); };

  const mut = (fn) => { setData(p => { const d = JSON.parse(JSON.stringify(p)); fn(d); return d; }); setStatus('⚠ Незбережено'); };
  const save = async () => {
    ...
    <div className={s.adminPage}>
        ...
        <div className={s.tabContent}>
          ...
          {!stats ? <p className={s.hint}>Завантаження...</p> : <>
            <div className={s.statsGrid}>
              ...
              ].map(([n,l]) => <div key={l} className={s.statCard}><div className={s.statN}>{n}</div><div className={s.statL}>{l}</div></div>)}
            </div>
      ...
      {tab === 'edit' && data && (
        <div className={s.tabContent}>
          ...
          <p className={s.hint}>Тягни ⠿ щоб змінити порядок · 💾 Зберегти щоб всі побачили зміни</p>
          {data.sections.map((sec, si) => (
            <SecEditor key={sec.id||si} sec={sec} si={si}
              ...
              onAdd={() => mut(d=>{d.sections[si].items.push({id:Date.now(),name:'Нова позиція',nameEn:'',price:'0 ₴',photo:'',description:'',descriptionEn:'',visible:true});})}
  ...
  return (
    <div className={`${s.secBox} ${isDragOver ? s.dragOver : ''}`} {...dragProps}>
      <div className={s.secHdr}>
        ...
        <button className={`${s.visBtn} ${sec.visible?s.visBtnOn:s.visBtnOff}`} onClick={()=>onSec('visible',!sec.visible)}>{sec.visible?'Видимо':'Приховано'}</button>
        <span className={s.secChev} onClick={onToggle}>{isOpen?'▴':'▾'}</span>
        <button className={s.delSec} onClick={onDelSec}>✕</button>
      </div>
      {isOpen && <div className={s.secBody}>
        <div className={s.secMeta}>
          ...
          <div className={s.fg} style={{flex:'0 0 60px'}}><label>Іконка</label><input value={sec.icon} onChange={e=>onSec('icon',e.target.value)}/></div>
        </div>
        {sec.items.map((item,ii) => (
          <ItemEditor key={item.id||ii} item={item} ii={ii}
            dragProps={itemDrag(ii)} isDragOver={itemOver===ii}
            onUpd={(f,v)=>onItem(ii,f,v)} onDel={()=>onDel(ii)}
          />
        ))}
    ...
    <div className={`${s.itemCard} ${isDragOver ? s.dragOver : ''}`} {...dragProps}>
      <div className={s.itemRow}>
        ...
        <div className={s.itemActs}>
          <button className={`${s.visToggle} ${item.visible?s.vOn:s.vOff}`} onClick={()=>onUpd('visible',!item.visible)}>{item.visible?'✓':'✗'}</button>
          <button className={s.expandBtn} onClick={()=>setExpanded(p=>!p)} title="Опис">✎</button>
      ...
      {expanded && (
        <div className={s.itemExpand}>
          <div className={s.descRow}>
            <div className={s.fg}><label>Опис UA</label><textarea rows={2} value={item.description||''} placeholder="Склад, особливості..." onChange={e=>onUpd('description',e.target.value)}/></div>
            <div className={s.fg}><label>Description EN</label><textarea rows={2} value={item.descriptionEn||''} placeholder="Ingredients..." onChange={e=>onUpd('descriptionEn',e.target.value)}/></div>
          </div>
          <div style={{marginTop:8,display:'flex',alignItems:'center',gap:10}}>
            ...
            {item.photo && !uploadErr && <span style={{fontSize:11,color:'var(--green)'}}>✓ Фото додано</span>}
          </div>
        </div>
      )}
    </div>
